      ...
      <div className="relative w-full aspect-video bg-gradient-to-br from-blue-50 to-green-50 dark:from-slate-800 dark:to-slate-900 rounded-lg overflow-hidden border-2 border-primary/20 shadow-lg">
        ...
        >
          ...
          {knowledgeLocations.map((location) => (
            ...
            >
              {/* Outer Glow */}
              <div
                className={`absolute inset-0 rounded-full blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-gradient-to-r ${getCategoryColor(location.category)}`}
                style={{
                  width: "48px",
                  height: "48px",
                  left: "-24px",
              ...
              {/* Point */}
              <div
                className={`relative w-12 h-12 rounded-full bg-gradient-to-r ${getCategoryColor(location.category)} shadow-lg hover:shadow-2xl transition-all duration-300 flex items-center justify-center border-2 border-white dark:border-slate-900 transform group-hover:scale-125`}
              >
        ...
        <div className="absolute top-4 left-4 glass dark:bg-white/10 dark:border-white/15 p-3 rounded-lg space-y-2 z-20 text-sm">
          ...
          <div className="space-y-1">
            {["Agriculture", "Alimentation", "Santé", "Construction", "Artisanat"].map((cat) => (
              <div key={cat} className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full bg-gradient-to-r ${getCategoryColor(cat)}`} />
          ...
          <div className="space-y-3">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-foreground/70">{selectedLocation.region}</p>
              </div>
              <span
                className={`px-3 py-1 rounded-full text-sm font-medium text-white bg-gradient-to-r ${getCategoryColor(selectedLocation.category)}`}
              >
            ...
            <div className="pt-2 border-t border-primary/20 dark:border-white/10">
              <p className="text-xs text-foreground/60">Par {selectedLocation.contributor}</p>
            </div>
          </div>
        </div>
      )}
