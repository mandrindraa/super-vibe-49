# Authentication System Architecture

## 🏗️ System Architecture Diagram

```
┌─────────────────────────────────────────────────────────────────┐
│                        User Browser                               │
├─────────────────────────────────────────────────────────────────┤
│                                                                   │
│  ┌──────────────────────┐      ┌──────────────────────┐          │
│  │   Login Page         │      │   Protected Pages    │          │
│  │  /auth/login         │      │   useAuthSession()   │          │
│  │                      │      │                      │          │
│  │ Email/Password Form  │      │ ┌────────────────┐  │          │
│  │ Google OAuth Button  │      │ │ Get user data  │  │          │
│  └──────┬───────────────┘      │ │ from session   │  │          │
│         │                      │ └────────────────┘  │          │
│         │ signIn()            │                      │          │
│         │                      └──────────────────────┘          │
│         └─────────────┬─────────────────────────────────┬────────┤
│                       │ JWT Token in Cookie              │       │
└───────────────────────┼─────────────────────────────────┼────────┘
                        │                                  │
                        ▼                                  ▼
        ┌───────────────────────────────────────────────────────────┐
        │           Next.js API Routes & Middleware                 │
        ├───────────────────────────────────────────────────────────┤
        │                                                             │
        │  /api/auth/[...nextauth]  ◄─── NextAuth.js Configuration  │
        │       ▼                                                     │
        │  ┌──────────────────┐     ┌──────────────────────┐        │
        │  │ Credentials      │     │ Google OAuth         │        │
        │  │ Provider         │     │ Provider             │        │
        │  │                  │     │                      │        │
        │  │ Email/Password   │     │ External: Google     │        │
        │  │ Validation       │     │ oauth2.googleapis.com        │
        │  └────────┬─────────┘     └──────────┬───────────┘        │
        │           │                          │                     │
        │           └──────────┬───────────────┘                     │
        │                      │                                     │
        │  /api/auth/signup    │   Create JWT Token                 │
        │  ▲                   ▼   Store in HTTP-only Cookie        │
        │  │         ┌──────────────────────┐                       │
        │  │         │  NextAuth Callbacks  │                       │
        │  │         │                      │                       │
        │  │         │ signIn()    ──────►  │                       │
        │  │         │ jwt()       ──────►  │                       │
        │  │         │ session()   ──────►  │                       │
        │  │         └────────┬─────────────┘                       │
        │  │                  │                                     │
        │  └──────────────────┘                                     │
        │                                                             │
        └────────────────────┬────────────────────────────────────────┘
                             │
                             ▼
        ┌───────────────────────────────────────────────────────────┐
        │                  Supabase                                  │
        ├───────────────────────────────────────────────────────────┤
        │                                                             │
        │  ┌─────────────────┐          ┌─────────────────┐         │
        │  │  auth.users     │          │   profiles      │         │
        │  ├─────────────────┤          ├─────────────────┤         │
        │  │ id (UUID)       │◄────────►│ id (FK)         │         │
        │  │ email           │          │ username        │         │
        │  │ encrypted_pwd   │          │ full_name       │         │
        │  │ user_metadata   │          │ avatar_url      │         │
        │  │ created_at      │          │ reputation_score│         │
        │  └─────────────────┘          │ bio, website    │         │
        │                               │ badges (JSONB)  │         │
        │                               │ created_at      │         │
        │                               └─────────────────┘         │
        │                                                             │
        └───────────────────────────────────────────────────────────┘
```

## 🔄 Authentication Flows

### Email/Password Login Flow

```
┌─────────┐
│  User   │
│ Enters  │
│Credentials
└────┬────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ signIn("credentials", {                 │
│   email: "user@example.com",            │
│   password: "password123"                │
│ })                                      │
└────┬────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ NextAuth Credentials Provider           │
│ authorize() method                      │
└────┬────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ Supabase Auth                           │
│ signInWithPassword()                    │
└────┬────────────────────────────────────┘
     │
     ├─► Validate credentials
     ├─► Check user exists
     └─► Get auth token

     ▼
┌─────────────────────────────────────────┐
│ Query profiles table                    │
│ Get user profile data                   │
└────┬────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ Return User Object                      │
│ {                                       │
│   id, email, name, image, username      │
│ }                                       │
└────┬────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ NextAuth Creates JWT Token              │
│ Encodes user data into token            │
└────┬────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ Store Token in HTTP-only Cookie         │
│ Set-Cookie: next-auth.session-token=... │
└────┬────────────────────────────────────┘
     │
     ▼
┌─────────────────────────────────────────┐
│ Redirect to Home Page                   │
│ User is now logged in                   │
└─────────────────────────────────────────┘
```

### Google OAuth Flow

```
┌─────────────┐
│ User Clicks │
│Google Button│
└────┬────────┘
     │
     ▼
┌──────────────────────────────────┐
│ signIn("google")                 │
│ Redirect to Google OAuth Screen  │
└────┬─────────────────────────────┘
     │
     ▼
┌──────────────────────────────────┐
│ User Authenticates with Google   │
│ Grants Permissions               │
└────┬─────────────────────────────┘
     │
     ▼
┌──────────────────────────────────┐
│ Google Returns                   │
│ - id                             │
│ - email                          │
│ - name                           │
│ - picture                        │
└────┬─────────────────────────────┘
     │
     ▼
┌──────────────────────────────────┐
│ NextAuth signIn() Callback       │
│ Check if profile exists          │
└────┬─────────────────────────────┘
     │
     ├─► Profile Exists?
     │   └─ YES: Update with latest data
     │
     └─► NO: Create new profile
         └─ Generate unique username
         └─ Insert into profiles table

     ▼
┌──────────────────────────────────┐
│ Create JWT Token                 │
│ with User Data                   │
└────┬─────────────────────────────┘
     │
     ▼
┌──────────────────────────────────┐
│ Store in HTTP-only Cookie        │
└────┬─────────────────────────────┘
     │
     ▼
┌──────────────────────────────────┐
│ Redirect to Home                 │
│ User is logged in                │
└──────────────────────────────────┘
```

## 🔐 Data Flow Diagram

```
Browser (Client)                 Server                         Database
    │                              │                              │
    ├─ useAuthSession() ─────────► getServerSession() ────────► JWT Token
    │  (client hook)               (server utility)             (in cookie)
    │                              │                              │
    │                              ├─ Verify JWT ────────────────┤
    │                              │                              │
    │◄─ Return User Data ◄────── Get Fresh Data ◄─────────────── profiles
    │   (from JWT token)         (on each request)              table
    │                              │                              │
    │◄─ Session Object ◄──────── Enrich JWT ◄─────────────────── Fetch
    │   {                        (jwt callback)                  user data
    │     user: {...},           │
    │     expires: ...           │
    │   }                        │
    │                            │
    │─ signIn() request ────────► NextAuth Handler ───────────► Supabase
    │                            │                              Auth
    │                            ├─► Validate Credentials
    │                            │
    │◄─ JWT Token ◄──────────── Create JWT ◄─ Get User
    │  (in cookie)             (jwt callback)  from Auth
    │                            │
    │                            ├─► Create Profile
    │                            └──► profiles table
```

## 📱 Component Integration

```
App Root
├── SessionProvider (NextAuth)
│   └── Providers (from app/providers.tsx)
│       └── QueryClientProvider
│           └── ThemeProvider
│               └── Layout
│                   ├── Header
│                   │   └── UserMenu (useSession)
│                   │
│                   ├── /auth/login
│                   │   └── Login Form
│                   │       ├── Credentials Form
│                   │       └── OAuth Buttons
│                   │
│                   ├── /auth/signup (future)
│                   │   └── Registration Form
│                   │       └── Call /api/auth/signup
│                   │
│                   └── Protected Pages
│                       └── useAuthSession
│                           └── Guard access
```

## 🔌 API Endpoints

```
NextAuth Endpoints (automatic)
├── GET  /api/auth/signin
├── GET  /api/auth/callback/google
├── GET  /api/auth/callback/credentials
├── POST /api/auth/signin
├── POST /api/auth/signout
├── GET  /api/auth/session
├── GET  /api/auth/csrf
└── GET  /api/auth/providers

Custom Endpoints
├── POST /api/auth/signup
│   ├── Input: { email, password, fullName }
│   ├── Returns: { user, message }
│   └── Creates: Supabase Auth + Profile
│
└── Protected Routes (require auth)
    └── Use requireAuth() utility
```

## 🔑 Token & Session Lifecycle

```
Timeline          Event                       Storage
──────────────────────────────────────────────────────────
T=0               User Login
                  JWT Created (30 days)
                  ├─ payload: {id, email, username, ...}
                  ├─ signed with NEXTAUTH_SECRET
                  └─ expires: T + 30 days

                  Token stored in cookie
                  ├─ HTTP-only
                  ├─ Secure (HTTPS)
                  └─ SameSite: Lax
                                               Cookie Storage

T=1min            Request to Protected Page
                  ├─ Cookie sent
                  ├─ JWT verified
                  └─ jwt() callback updates
                      user data from DB       Database Query
                                               profiles table

T=30 days         Token Expires
                  ├─ JWT invalid
                  ├─ User logged out
                  └─ Redirect to login        Clean Cookie

T=31 days         New Login Required          [User logs in again]
```

## 🛡️ Security Layers

```
Layer 1: Transport
├── HTTPS only (production)
└── HTTP-only cookies (no JS access)

Layer 2: Token
├── JWT signed with NEXTAUTH_SECRET
├── Payload includes user.id
├── Expires after 30 days
└── Refreshed on new requests

Layer 3: Database
├── Supabase Auth (encrypted passwords)
├── Separate profiles table
├── Row Level Security (RLS)
└── Indexed lookups (username)

Layer 4: API
├── CORS protection
├── CSRF tokens (NextAuth)
├── Input validation (Zod)
└── Rate limiting (optional)

Layer 5: Code
├── Type safety (TypeScript)
├── No sensitive data in JWT
├── Secure key usage
└── Error handling (no data leaks)
```

---

This architecture ensures:
✅ Security (encrypted, tokens, RLS)
✅ Scalability (JWT strategy)
✅ Performance (indexed queries)
✅ Maintainability (clear separation)
✅ Type Safety (full TypeScript)
